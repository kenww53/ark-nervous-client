 ...
 *
 * Services use this to verify a specific peer's outbound HMAC signatures
 * (defaults to NESHAMAH). Returns the configured secret only when the
 * incoming service name and computed secret_id match.
 *
 * Env-var resolution order (Phase 4.5c follow-up #5 — naming unification):
 *   NERVOUS_INBOUND_<PEER>_SECRET  — canonical name; <PEER> is the
 *                                     uppercased expectedService (e.g.
 *                                     NERVOUS_INBOUND_NESHAMAH_SECRET).
 *   NERVOUS_SIGNAL_SECRET          — legacy name; same meaning. Continues
 *                                     to work for backward compatibility
 *                                     during the temple-wide migration.
 *
 * The secret_id is COMPUTED from the secret bytes at boot
 * (SHA256(decoded)[:8]); a separate NERVOUS_SECRET_ID env var is no longer
 * required. If present, it must match the computed value or this function
 * logs a warning and falls back to the computed (authoritative) value.
 *
 * Usage:
...
function createEnvSecretLookup(expectedService = 'neshamah') {
    const peerEnvKey = `NERVOUS_INBOUND_${expectedService.toUpperCase()}_SECRET`;
    const secret = process.env[peerEnvKey] || process.env.NERVOUS_SIGNAL_SECRET || null;
    if (!secret) {
        return async () => null;
    }
    let computedId;
    try {
        computedId = computeSecretId(secret);
    }
    catch {
        return async () => null;
    }
    // Legacy NERVOUS_SECRET_ID was a hint; the secret is the authority. If
    // present and mismatched, warn but trust the computed id.
    const legacyId = process.env.NERVOUS_SECRET_ID;
    if (legacyId && legacyId !== computedId) {
        // eslint-disable-next-line no-console
        console.warn(`[createEnvSecretLookup] NERVOUS_SECRET_ID env (${legacyId}) does not match SHA256(secret) (${computedId}) — using computed`);
    }
    return async (service, id) => {
        if (service === expectedService && id === computedId) {
